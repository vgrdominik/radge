...
func physics_process(delta: float) -> void:
	...
	# Rotation
	if move_direction:
		var target_direction: = player.transform.looking_at(player.transform.origin + move_direction, Vector3.UP)
...
func enter(msg: Dictionary = {}) -> void:
	player.camera.connect("aim_fired", self, "_on_CameraRig_aim_fired")


func exit(target_state: State) -> void:
	player.camera.disconnect("aim_fired", self, "_on_CameraRig_aim_fired")
...
func _on_CameraRig_aim_fired(target_vector: Vector3) -> void:
